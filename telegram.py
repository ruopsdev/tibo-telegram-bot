...
bar_members = {
    "41365750": {"username": "ChudakovSergey", "first": "Sergey"},
    "670403191": {"username": "elijah_here", "first": "Илья", "last": "Полосков"},
    "1006923818": {"first": "James", "last": "Touchet"},
    "61049840": {"first": "Pavel", "last": "S"},
    "652907968": {"first": "Nikita"},
}
...
def command_start(m):
    try:
        ...
        print(
            f"Command_/start handler triggered! Chat ID: {cid}, Message: {m.text}, Language: {lang}"
        )
...
)  # qinggan = 情感 (emotion), fenxi = 分析 (analysis) in Chinese Pinyin
...
@bot.message_handler(
    func=lambda m: m.text and not m.text.startswith("/") and contains_chinese(m.text)
)
...
@bot.message_handler(
    commands=["detect", "jianceyu"]
)  # jianceyu = 检测语言 (detect language in Chinese Pinyin)
def detect_language_handler(message: Message):
    ...
    if not LANGDETECT_AVAILABLE:
        if lang == "zh":
            ...
            bot.reply_to(
                message,
                "Language detection not available. Install langdetect: pip install langdetect",
            )
        return
...
def translate_handler(message: Message):
    ...
    try:
        ...
        if lang == "zh":
            bot.reply_to(
                message, "翻译功能不可用。请安装：pip install googletrans==4.0.0rc1"
            )
        else:
            bot.reply_to(
                message,
                "Translation not available. Install googletrans: pip install googletrans==4.0.0rc1",
            )
...
def translate_reply(message, translator):
    ...
    try:
        ...
        if len(parts) < 2:
            ...
            response = (
                f"🌐 Translation (auto-detected {result.src} → en):\n\n{result.text}"
            )
        else:
    ...
    try:
        ...
        if json_string:
            ...
            if update:
                ...
                if update.message:
                    print(
                        f"Update contains message: {update.message.text if update.message.text else 'No text'}"
                    )
...
def webhook():
    try:
        bot.remove_webhook()
        webhook_url = (
            f"https://tibo-telegram-bot.onrender.com/{TIBO_TELEGRAM_BOT_TOKEN}"
        )
        ...
        if response.status_code == 200 or response.status_code == 201:
            ...
            try:
                ...
                if "service" in restart_data:
                    ...
                    service_status = (
                        service_obj.get("status")
                        or service_obj.get("serviceDetails", {}).get("status")
                        or service_obj.get("deploy", {}).get("status")
                ...
                return (
                    f"Service restart initiated successfully.<br>Current Status: {service_status}<br>HTTP Status: {response.status_code}",
                    200,
                )
                ...
                return (
                    f"Service restart initiated successfully. Status code: {response.status_code}",
                    200,
                )
        else:
            return (
                f"Failed to restart service. Status: {response.status_code}, Response: {response.text}",
        ...
        if response.status_code == 200:
            ...
            if "service" in service_data:
                ...
                if "serviceDetails" in service_obj:
                    status_info["details"] = service_obj["serviceDetails"].get(
                        "status", "Unknown"
                    )
            else:
            ...
            return {
                "error": f"Failed to get service status. HTTP {response.status_code}",
